...
export class InMemoryTodoRepo implements TodoRepo {
  private todos: Todo[] = [
    ...
    { id: 4, text: "Important todoooo", completed: false, listId: "important" },
  ];
  ...
  public async editTodo(id: number, text: string, listId?: string): Promise<void> {
    if (text.length < 3) throw new Error("Todo needs to be longer than 3 characters.");
    ...
    if (listId) {
      this.todos[found].listId = listId;
    }
  }
